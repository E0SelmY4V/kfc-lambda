...
const a = not(bTrue); // 怎么能验证 a 是 bFalse ？

const n1 = f => x => f(x);
const plus = a => b => f => x => a(f)(b(f)(x));

const temp = plus(n1) // 怎么能把这个结果转换为 Lambda 表达式？

const multi = a => b => a(plus(b))(bFalse)
const pred = n => f => x => n(p => h => h(p(f)))(_ => x)(n => bFalse);
...
```ts
import { jsifier, yC, Lambda } from 'lambda-calculus-with-js';
...
有的时候我们希望表示局部的 lambda 表达式，会出现没有被定义过的参数，也就是自由标识符。
